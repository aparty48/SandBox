...
namespace Digger
{
    public class VoxelChunk : MonoBehaviour
    {
            ...
            ref float[] verticalNormalArray)
        {
            ...
            for (var xi = 0; xi < sizeVox; ++xi) {
                for (var zi = 0; zi < sizeVox; ++zi) {
                    heightArray[xi * sizeVox + zi] = heightFeeder.GetHeight(chunkVoxelPosition.x + xi, chunkVoxelPosition.z + zi);
                    verticalNormalArray[xi * sizeVox + zi] = heightFeeder.GetVerticalNormal(chunkVoxelPosition.x + xi, chunkVoxelPosition.z + zi);
                }
            }
        ...
        public void UnCutAllVertically()
        {
            ...
            for (var xi = 0; xi < sizeVox; ++xi) {
                for (var zi = 0; zi < sizeVox; ++zi) {
                    var pos = new Vector3(xi * hScale.x, 0, zi * hScale.z);
