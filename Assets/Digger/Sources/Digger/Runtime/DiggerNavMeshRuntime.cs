    ...
    public class DiggerNavMeshRuntime : MonoBehaviour
    {
        ...
        public void CollectNavMeshSources()
        {
            for (var i = 0; i < surfaces.Length; i++) {
                ...
                var initialNavMeshBuildSources = surface.CollectSources();
                ...
                if (surface.useGeometry == NavMeshCollectGeometry.RenderMeshes) {
                    var batchingStaticObjects = initialNavMeshBuildSources
                                                .Where(x => x.component != null && !(x.component is Terrain) && GameObjectUtility.AreStaticEditorFlagsSet(x.component.gameObject, StaticEditorFlags.BatchingStatic))
                                                .Select<NavMeshBuildSource, GameObject>(x => x.component.gameObject)
                                                .Take(100);
                    foreach (var batchingStaticObject in batchingStaticObjects) {
                        Debug.LogWarning($"NavMesh sources include object \"<b>{batchingStaticObject.name}</b>\" that has 'Batching Static' enabled. <b>This will cause errors</b> if NavMesh is " +
                ...
                initialBoundsPerSurface[i] = surface.CalculateWorldBounds(initialNavMeshBuildSources);
