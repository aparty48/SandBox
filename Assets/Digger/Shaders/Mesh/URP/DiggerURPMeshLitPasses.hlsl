...
void SplatmapMix(float3 positionWS, float3 normal, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    half4 opacityAsDensity = saturate((half4(diffAlbedo[0].a, diffAlbedo[1].a, diffAlbedo[2].a, diffAlbedo[3].a) - (half4(1.0, 1.0, 1.0, 1.0) - splatControl)) * 2);
...
#ifdef _MASKMAP
    masks[0] = lerp(masks[0], SampleTriplanar(IN.positionWS, IN.normal, _Mask0, sampler_Mask0, _Splat0_ST.xy, _Splat0_ST.zw), hasMask.x);
    masks[1] = lerp(masks[1], SampleTriplanar(IN.positionWS, IN.normal, _Mask1, sampler_Mask0, _Splat0_ST.xy, _Splat0_ST.zw), hasMask.y);
    masks[2] = lerp(masks[2], SampleTriplanar(IN.positionWS, IN.normal, _Mask2, sampler_Mask0, _Splat0_ST.xy, _Splat0_ST.zw), hasMask.z);
    masks[3] = lerp(masks[3], SampleTriplanar(IN.positionWS, IN.normal, _Mask3, sampler_Mask0, _Splat0_ST.xy, _Splat0_ST.zw), hasMask.w);
#endif
...
half4 SplatmapFragment(Varyings IN) : SV_TARGET
{
    ...
    SplatmapMix(IN.positionWS, IN.normal, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS);
