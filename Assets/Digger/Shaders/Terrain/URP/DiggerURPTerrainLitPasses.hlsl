...
half4 SampleTriplanar(float3 positionWS, float3 normal, Texture2D tex, SamplerState samp, float2 tile, float2 offset) {
    float2 y0 = positionWS.zy * tile + offset;
    float2 x0 = positionWS.xz * tile + offset;
    float2 z0 = positionWS.xy * tile + offset;
    float4 cX0 = SAMPLE_TEXTURE2D(tex, samp, y0);
    float4 cY0 = SAMPLE_TEXTURE2D(tex, samp, x0);
    float4 cZ0 = SAMPLE_TEXTURE2D(tex, samp, z0);
    float4 side0 = lerp(cX0, cZ0, abs(normal.z));
    return lerp(side0, cY0, abs(normal.y));    
}

void SplatmapMix(float3 positionWS, float3 normal, float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    float2 _TerrainSizeInv = float2(_TerrainWidthInv, _TerrainHeightInv);
    half4 diffAlbedo[4];

    diffAlbedo[0] = SampleTriplanar(positionWS, normal, _Splat0, sampler_Splat0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw);
    diffAlbedo[1] = SampleTriplanar(positionWS, normal, _Splat1, sampler_Splat0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw);
    diffAlbedo[2] = SampleTriplanar(positionWS, normal, _Splat2, sampler_Splat0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw);
    diffAlbedo[3] = SampleTriplanar(positionWS, normal, _Splat3, sampler_Splat0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw);
...
#ifdef _NORMALMAP
    half3 nrm = 0.0f;
    nrm += splatControl.r * UnpackNormalScale(SampleTriplanar(positionWS, normal, _Normal0, sampler_Normal0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), _NormalScale0);
    nrm += splatControl.g * UnpackNormalScale(SampleTriplanar(positionWS, normal, _Normal1, sampler_Normal0, _Splat1_ST.xy * _TerrainSizeInv, _Splat1_ST.zw), _NormalScale1);
    nrm += splatControl.b * UnpackNormalScale(SampleTriplanar(positionWS, normal, _Normal2, sampler_Normal0, _Splat2_ST.xy * _TerrainSizeInv, _Splat2_ST.zw), _NormalScale2);
    nrm += splatControl.a * UnpackNormalScale(SampleTriplanar(positionWS, normal, _Normal3, sampler_Normal0, _Splat3_ST.xy * _TerrainSizeInv, _Splat3_ST.zw), _NormalScale3);
...
#ifdef _MASKMAP
    float2 _TerrainSizeInv = float2(_TerrainWidthInv, _TerrainHeightInv);
    masks[0] = lerp(masks[0], SampleTriplanar(IN.positionWS, IN.normal, _Mask0, sampler_Mask0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), hasMask.x);
    masks[1] = lerp(masks[1], SampleTriplanar(IN.positionWS, IN.normal, _Mask1, sampler_Mask0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), hasMask.y);
    masks[2] = lerp(masks[2], SampleTriplanar(IN.positionWS, IN.normal, _Mask2, sampler_Mask0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), hasMask.z);
    masks[3] = lerp(masks[3], SampleTriplanar(IN.positionWS, IN.normal, _Mask3, sampler_Mask0, _Splat0_ST.xy * _TerrainSizeInv, _Splat0_ST.zw), hasMask.w);
#endif
...
half4 SplatmapFragment(Varyings IN) : SV_TARGET
{
    ...
    SplatmapMix(IN.positionWS, IN.normal, IN.uvMainAndLM, IN.uvSplat01, IN.uvSplat23, splatControl, weight, mixedDiffuse, defaultSmoothness, normalTS);
